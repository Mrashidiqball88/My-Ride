export const PAKISTAN_ONLY_MESSAGE = "Please select a location inside Pakistan.";

// Locally vendored WGS84 national boundary ring. A polygon is required here:
// a rectangular latitude/longitude range would admit locations in neighboring
// countries, despite the platform operating only inside Pakistan.
const PAKISTAN_BOUNDARY: ReadonlyArray<readonly [number, number]> = [
...
function pointIsOnSegment(x: number, y: number, x1: number, y1: number, x2: number, y2: number): boolean {
  const cross = (x - x1) * (y2 - y1) - (y - y1) * (x2 - x1);
  if (Math.abs(cross) > 1e-9) return false;
  return (x - x1) * (x - x2) + (y - y1) * (y - y2) <= 1e-9;
}

export function isWithinPakistanCoordinates(latitude: number, longitude: number): boolean {
  ...
  for (let i = 0, j = PAKISTAN_BOUNDARY.length - 1; i < PAKISTAN_BOUNDARY.length; j = i++) {
    ...
    if (pointIsOnSegment(longitude, latitude, xi, yi, xj, yj)) return true;
