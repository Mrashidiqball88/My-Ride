(function exposePakistanGeofence(root, factory) {
  const api = factory();
  if (typeof module === 'object' && module.exports) module.exports = api;
  root.MyRidePakistanGeofence = api;
})(typeof globalThis !== 'undefined' ? globalThis : this, function createPakistanGeofence() {
  'use strict';

  const PAKISTAN_ONLY_MESSAGE = 'Please select a location inside Pakistan.';

  // Locally vendored national boundary ring (WGS84 longitude/latitude). This is
  // intentionally a polygon check rather than a loose latitude/longitude box:
  // a bounding box would incorrectly admit locations in neighboring countries.
  const PAKISTAN_BOUNDARY = [
  ...
  function isFiniteWorldCoordinate(lat, lng) {
  ...
  function pointInPolygon(lat, lng) {
    let inside = false;
    for (let i = 0, j = PAKISTAN_BOUNDARY.length - 1; i < PAKISTAN_BOUNDARY.length; j = i++) {
      const [xi, yi] = PAKISTAN_BOUNDARY[i];
      const [xj, yj] = PAKISTAN_BOUNDARY[j];
      if (pointIsOnSegment(lng, lat, xi, yi, xj, yj)) return true;
  ...
  function isWithinPakistan(locationOrLat, suppliedLng) {
    const lat = Number(typeof locationOrLat === 'object' ? locationOrLat?.lat : locationOrLat);
    const lng = Number(typeof locationOrLat === 'object' ? locationOrLat?.lng : suppliedLng);
    return isFiniteWorldCoordinate(lat, lng) && pointInPolygon(lat, lng);
  }

  return Object.freeze({ PAKISTAN_ONLY_MESSAGE, isWithinPakistan });
});